...
def check_authorial_voice(html_path: str) -> int:
    ...
    for i, (pi, wtf, cp, dm) in enumerate(blocks):
        ...
        for field_name, field_val in [("PEDAGOGICAL INTENT", pi),
            ...
            for lp in lazy_patterns:
                if lp in lower:
                    violations.append({
                        ...
                        "text": field_val[:80],
                        "severity": "error" if "pedagogical" in field_name.lower() else "warning"
                    })

        # Check 2: WHY THIS FEATURE too thin (<25 substantive chars)
        wtf_stripped = wtf.replace("static", "").replace("fragment fade-up", "").replace("fragment", "").strip()
        ...
        dm_has_technical = any(t in dm.lower() for t in [
            "html", "css", "flex layout", "data-background", "data-audio-src",
            "data-autoplay", "reveal.js's", "data-vocab-audio", "javascript",
            "a-why class", "call startembeddedcontent", "queryselector",
        ])
        dm_has_rationale = any(t in dm.lower() for t in [
            "without", "if removed", "so the student", "so the learner",
            "the teacher can", "students can", "so that the",
        ])
        ...
        is_answer = any(t in pi.lower() for t in ["answer", "a-row", "badge", "correction", "comprehension question"])
        if is_answer:
            identical_count = 0
            prev_text = None
