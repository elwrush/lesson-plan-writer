...
- **CRITICAL: Only include items explicitly stated by the user in materials.** Do NOT add derived/ancillary assets (e.g., logo files, background images, downloaded photos, character composites, slide assets). Materials = what the teacher needs to prepare or bring to class: books, handouts, videos, audio files. Supplied images (Pixabay, Wikimedia, character composites, author photos) are slide implementation details, not materials. Never reference them.
