...
| `.title-row` | Fenced div `::: {.title-row}` | Dark shield background, white-space nowrap |
| `.slide-title` | Span `[text]{.slide-title}` | 1.8em bold white |
| `.shield` | Fenced div `::: {.shield}` | Dark semi-transparent background for text on images |
| `.cta-text` | Span `[text]{.cta-text}` | Yellow bold |
| `.cefr-badge` | Span `[B2]{.cefr-badge}` | Blue badge (currently unused) |
| `.answer-reveal` | Fenced div `::: {.fragment .answer-reveal}` | Hidden until clicked, then yellow bold |
...
5. **Text shields on image backgrounds** — use `::: {.shield}` or `::: {.title-row}`
